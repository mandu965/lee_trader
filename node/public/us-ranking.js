...
function renderTable(rows) {
  ...
  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="14" class="empty">해당 조건의 종목이 없습니다.</td></tr>';
    return;
  }
  ...
  const html = rows.map((r, idx) => {
    ...
    const detail = r.score_detail && typeof r.score_detail === "object" ? r.score_detail : null;
    const prob = r.probability ?? detail?.probability ?? null;
    const probStr = prob != null && Number.isFinite(Number(prob)) ? fmt(Number(prob) * 100, 1) + "%" : "—";
      ...
      <tr id="${detailId}" class="detail-row">
        <td colspan="14" class="detail-cell">
          ...
          <div class="detail-grid">
            ...
            <div class="detail-item"><div class="lbl">Probability</div><div class="val">${probStr}</div></div>
...
async function loadRanking() {
  ...
  document.getElementById("rankBody").innerHTML = '<tr><td colspan="14" class="empty">로딩 중…</td></tr>';
  ...
  try {
    ...
    document.getElementById("rankBody").innerHTML = '<tr><td colspan="14" class="empty">—</td></tr>';
  } finally {
    state.loading = false;
  }
}
